...
**To use Enterprise features with a local router instance, the router must connect to GraphOS to obtain its [entitlement](#the-enterprise-entitlement).** It _can_ do so while using a locally composed supergraph schema.
